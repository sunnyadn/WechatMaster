...
public class UserProfileActivity extends BaseActivity implements View.OnClickListener {
    ...
    public void asyncFetchUserInfo(String username){
        WeHelper.getInstance().getUserProfileManager().asyncGetUserInfo(username, new EMValueCallBack<EaseUser>() {

            @Override
            public void onSuccess(EaseUser user) {
                if (user != null) {
                    WeHelper.getInstance().saveContact(user);
                    if(isFinishing()){
                        return;
                    }
    ...
    private void updateRemoteNick(final String nickName) {
        ...
        new Thread(new Runnable() {
            ...
            @Override
            public void run() {
                boolean updatenick = WeHelper.getInstance().getUserProfileManager().updateCurrentUserNickName(nickName);
    ...
    private void uploadUserAvatar(final byte[] data) {
        ...
        new Thread(new Runnable() {
            ...
            @Override
            public void run() {
                final String avatarUrl = WeHelper.getInstance().getUserProfileManager().uploadUserAvatar(data);
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        dialog.dismiss();
